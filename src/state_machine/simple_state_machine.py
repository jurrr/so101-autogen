...
class SimpleGraspingStateMachine:
    ...
    def __init__(self, world, robot, ik_controller, gripper_controller, 
        ...
        self.close_angle_min = grasp_config.get('close_angle_percent_min', 0.0001)
        self.close_angle_max = grasp_config.get('close_angle_percent_max', 0.0001)

        # Load gripper approach parameters from object_geometry config
        try:
            ...
            self.xy_approach_offset = gripper_approach.get('xy_approach_offset', -0.005)
            self.grasp_height_offset = gripper_approach.get('grasp_height_offset', 0.015)
            logger.info(f"✅ Loaded gripper approach params: XY offset={self.xy_approach_offset*1000:.1f}mm, grasp_height={self.grasp_height_offset*1000:.1f}mm")
        except Exception as e:
            self.xy_approach_offset = -0.005
            self.grasp_height_offset = 0.015
            logger.warning(f"⚠️ Could not load gripper approach params from config: {e}, using defaults")
    ...
    def _start_approach(self):
        ...
        try:
            ...
            # Use configured XY offset to avoid gripper edge collision with cube edge
            # Apply offset in X direction (gripper approaches from the side, green ray is -X)
            approach_pos = np.array([
                target_pos[0] + self.xy_approach_offset,  # Offset in X (gripper side approach)
                target_pos[1], 
                self.approach_height
            ])
            
            self._start_smooth_move(approach_pos, self.travel_horizontal_speed)
            print(f"🚀 Approaching target: [{target_pos[0]:.3f}, {target_pos[1]:.3f}] with X offset={self.xy_approach_offset*1000:.1f}mm -> Height {self.approach_height}m")
        except Exception as e:
